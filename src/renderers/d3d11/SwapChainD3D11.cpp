...
void SwapChainRenderTargetsD3D11::Init(const SwapChainD3D11 & sc, const int width, const int height)
{
    ...
    HRESULT hr = sc.swap_chain->GetBuffer(0, __uuidof(ID3D11Texture2D), (void**)&back_buffer_tex);
    if (FAILED(hr))
    {
        GameInterface::Errorf("Failed to get framebuffer from SwapChain!");
    }

    hr = sc.device->CreateRenderTargetView(back_buffer_tex, nullptr, framebuffer_rtv.GetAddressOf());
    if (FAILED(hr))
    {
    ...
    desc_depth.Width                = render_target_width;
    desc_depth.Height               = render_target_height;
    ...
    hr = sc.device->CreateTexture2D(&desc_depth, nullptr, depth_stencil_texture.GetAddressOf());
    if (FAILED(hr))
    {
    ...
    hr = sc.device->CreateDepthStencilView(depth_stencil_texture.Get(), &desc_dsv, depth_stencil_view.GetAddressOf());
    if (FAILED(hr))
    {
    ...
    sc.context->OMSetRenderTargets(1, framebuffer_rtv.GetAddressOf(), depth_stencil_view.Get());

    // Setup a default viewport:
    D3D11_VIEWPORT vp = {};
    vp.Width    = static_cast<float>(render_target_width);
    vp.Height   = static_cast<float>(render_target_height);
    vp.MinDepth = 0.0f;
    vp.MaxDepth = 1.0f;
    vp.TopLeftX = 0;
    vp.TopLeftY = 0;
