...
void UploadContextD3D11::UploadTextureImmediate(const TextureUploadD3D11 & upload_info)
{
    ...
    // NOTE: Only mip-level 0 is updated (assume texture is not mipmapped)

    auto * texture_resource = upload_info.texture->m_resource.Get();
    const UINT mip_index = 0;                     // no mips/slices
    const UINT row_pitch = upload_info.width * 4; // RGBA

    m_device->context->UpdateSubresource(texture_resource, mip_index, nullptr, upload_info.pixels, row_pitch, 0);
}
