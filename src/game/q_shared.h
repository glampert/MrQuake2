...
// These warnings are raised by the Quake2 code, suppressing since there are too many to fix.
#ifdef _MSC_VER
...
#if defined(__GNUC__) || defined(__clang__)
    #define Q_UNUSED_ARG(x) __attribute__((__unused__)) x
#elif defined(_MSC_VER)
    #define Q_UNUSED_ARG(x) __pragma(warning(suppress: 4100)) x
#else // !GNU && !Clang && !MSVC
    #define Q_UNUSED_ARG(x) /*unused*/ x
#endif // GNU/Clang/MSVC

typedef uintptr_t quptr;
typedef uint8_t   qbyte;
typedef bool      qboolean;

#define Q_istrlen(s) ((int)strlen(s))
...
// game print flags/levels
typedef enum
{
    PRINT_ALL    = -1, // internal
...
typedef struct usercmd_s
{
    ...
    qbyte impulse;    // remove?
    qbyte lightlevel; // light level the player is standing on
} usercmd_t;
