...
{
	// LAMPERT:
	// Modified to use multiple buffers, like in Doom3.
    enum
    {
...
void Info_SetValueForKey(char * s, char * key, char * value)
{
    ...
    if (Q_istrlen(key) > MAX_INFO_KEY - 1 || Q_istrlen(value) > MAX_INFO_KEY - 1)
    {
    ...
    if (Q_istrlen(newi) + Q_istrlen(s) > maxsize)
    {
