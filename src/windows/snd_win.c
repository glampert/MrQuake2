...
typedef HRESULT (WINAPI * PFN_DirectSoundCreate)(GUID FAR * lpGUID, LPDIRECTSOUND FAR * lplpDS, IUnknown FAR * pUnkOuter);
static PFN_DirectSoundCreate pDirectSoundCreate = NULL;
...
static void DS_DestroyBuffers(void)
{
    ...
    if (pDS)
    {
        ...
        pDS->lpVtbl->SetCooperativeLevel(pDS, winquake.hwnd, DSSCL_NORMAL);
    }
...
static qboolean DS_CreateBuffers(void)
{
    ...
    if (DS_OK != pDS->lpVtbl->SetCooperativeLevel(pDS, winquake.hwnd, DSSCL_EXCLUSIVE))
    {
        Com_Printf("failed\n");
        FreeSound();
        return false;
    ...
    else
    {
        ...
        if (DS_OK != pDS->lpVtbl->SetCooperativeLevel(pDS, winquake.hwnd, DSSCL_WRITEPRIMARY))
        {
            Com_Printf("failed\n");
            FreeSound();
            return false;
...
static sndinitstat SNDDMA_InitDirect(void)
{
    ...
    if (!hInstDS)
    {
        ...
        Com_DPrintf("ok\n");
        pDirectSoundCreate = (PFN_DirectSoundCreate)GetProcAddress(hInstDS, "DirectSoundCreate");
...
void S_Activate(qboolean active)
{
    if (active)
    {
        if (pDS && winquake.hwnd && snd_isdirect)
        {
            DS_CreateBuffers();
        }
    }
