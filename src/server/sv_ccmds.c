...
void SV_WriteServerFile(qboolean autosave)
{
    ...
    if (!autosave)
    {
        ...
        strncat(comment, sv.configstrings[CS_NAME], sizeof(comment) - 1 - Q_istrlen(comment));
    }
    else
    { // autosaved
    ...
    for (var = cvar_vars; var; var = var->next)
    {
        ...
        if (Q_istrlen(var->name) >= sizeof(name) - 1 || Q_istrlen(var->string) >= sizeof(string) - 1)
        {
...
void SV_ConSay_f(void)
{
    ...
    {
        ++p; // skip the first
        strncat(text, p, Q_istrlen(p) - 1); // copy all except the last char (")
    }
    else
    {
        strcat(text, p);
