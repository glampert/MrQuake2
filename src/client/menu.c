    ...
    */
    for (i = 0, y = viddef.height - ((cls.realtime - credits_start_time) / 40.0F); credits[i] && y < viddef.height; y += 10, i++)
    {
        ...
        for (j = 0; credits[i][j + stringoffset]; j++)
        {
            ...
            x = (viddef.width - Q_istrlen(credits[i]) * 8 - stringoffset * 8) / 2 + (j + stringoffset) * 8;
...
void StartServer_MenuInit(void)
{
    ...
    */
    ...
    for (i = 0; i < nummaps; i++)
    {
        ...
        l = Q_istrlen(shortname);
        for (j = 0; j < l; j++)
            shortname[j] = toupper(shortname[j]);
        strcpy(longname, COM_Parse(&s));
        Com_sprintf(scratch, sizeof(scratch), "%s\n%s", longname, shortname);

        mapnames[i] = Z_Malloc(Q_istrlen(scratch) + 1);
...
// filesys.c
extern char ** FS_ListFiles(char * findname, int * numfiles, unsigned musthave, unsigned canthave);
    ...
    for (i = 0; i < npms; i++)
    {
        ...
        for (s = 0, k = 0; k < npcxfiles - 1; k++)
        {
            if (!strstr(pcxnames[k], "_i.pcx"))
            {
                ...
                {
                    char * c;
                    char * a = strrchr(pcxnames[k], '/');
                    char * b = strrchr(pcxnames[k], '\\');
...
qboolean PlayerConfig_MenuInit(void)
{
    ...
    s_player_name_field.cursor = Q_istrlen(name->string);
