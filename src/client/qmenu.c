...
qboolean Field_Key(menufield_s * f, int key)
{
    ...
    */
    ...
    case K_BACKSPACE:
        if (f->cursor > 0)
        {
            memmove(&f->buffer[f->cursor - 1], &f->buffer[f->cursor], Q_istrlen(&f->buffer[f->cursor]) + 1);
            f->cursor--;
    ...
    case K_KP_DEL:
    case K_DEL:
        memmove(&f->buffer[f->cursor], &f->buffer[f->cursor + 1], Q_istrlen(&f->buffer[f->cursor + 1]) + 1);
        break;
...
void Menu_DrawStringR2L(int x, int y, const char * string)
{
    ...
    for (i = 0; i < Q_istrlen(string); i++)
    {
        re.DrawChar((x - i * 8), y, string[Q_istrlen(string) - i - 1]);
    }
}
...
void Menu_DrawStringR2LDark(int x, int y, const char * string)
{
    ...
    for (i = 0; i < Q_istrlen(string); i++)
    {
        re.DrawChar((x - i * 8), y, string[Q_istrlen(string) - i - 1] + 128);
    }
}
