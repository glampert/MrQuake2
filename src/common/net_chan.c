...
qbyte net_message_buffer[MAX_MSGLEN] = {0};
...
void Netchan_OutOfBandPrint(int net_socket, netadr_t adr, char * format, ...)
{
    ...
    Netchan_OutOfBand(net_socket, adr, Q_istrlen(string), (qbyte *)string);
}
...
void Netchan_Setup(netsrc_t sock, netchan_t * chan, netadr_t adr, int port)
{
    ...
    chan->qport = port;
    chan->last_received = sys_curtime;
