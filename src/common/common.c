...
void Com_Printf(const char * fmt, ...)
{
    ...
    if (rd_target)
    {
        if ((Q_istrlen(msg) + Q_istrlen(rd_buffer)) > (rd_buffersize - 1))
        {
            rd_flush(rd_target, rd_buffer);
            *rd_buffer = 0;
        }
...
=============
*/
void Com_Error(error_level_t code, const char * fmt, ...)
{
    va_list argptr;
...
void COM_InitArgv(int argc, char ** argv)
{
    ...
    for (i = 0; i < argc; i++)
    {
        if (!argv[i] || Q_istrlen(argv[i]) >= MAX_TOKEN_CHARS)
        {
            com_argv[i] = "";
        }
        else
...
/// just for debugging
int memsearch(const qbyte * start, int count, int search)
{
    int i;
    for (i = 0; i < count; i++)
    {
...
====================
*/
qbyte COM_BlockSequenceCheckByte(qbyte * base, int length, int sequence, int challenge)
{
